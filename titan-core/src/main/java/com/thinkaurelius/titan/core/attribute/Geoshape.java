...
public class Geoshape {
    ...
    public static final Geoshape polygon(List<double[]> coordinates) {
        ...
        for (int i=0; i<coordinates.size(); i++) {
            ...
            Preconditions.checkArgument(coordinate.length==2 && isValidCoordinate(coordinate[1],coordinate[0]),"Invalid coordinate provided");
            points[i] = new Coordinate(coordinate[0],  coordinate[1]);
        }
        GeometryFactory factory = new GeometryFactory();
        return new Geoshape(CTX.makeShape(factory.createPolygon(points)));
    }
